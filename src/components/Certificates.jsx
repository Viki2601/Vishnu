...
const sectionReveal = {
    ...
    visible: { opacity: 1, scale: 1, y: 0, transition: { type: 'spring', stiffness: 60, damping: 18, duration: 1.8, }, },
};
                ...
                <div className="w-full max-w-6xl mx-auto grid md:grid-cols-2 gap-16 items-center relative z-10">
                    ...
                    <motion.div variants={rightReveal} initial="hidden" animate={isInView ? 'visible' : 'hidden'} className="flex flex-col gap-4">
                        {certificateData?.map((cert, index) => (
                            <motion.div key={index} custom={index} variants={cardVariants} initial="hidden" animate={isInView ? 'visible' : 'hidden'} onMouseEnter={() => setHovered(index)} onMouseLeave={() => setHovered(null)} className="overflow-hidden cursor-default" style={{ borderRadius: 'var(--radius-lg)', border: '1px solid rgba(192, 38, 211, 0.15)', background: 'rgba(5, 5, 12, 0.82)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.03)', transition: 'border-color 0.3s ease, box-shadow 0.3s ease', }} whileHover={{ borderColor: 'var(--accent-light)', boxShadow: '0 0 28px rgba(6, 182, 212, 0.18), inset 0 1px 0 rgba(6, 182, 212, 0.06)', scale: 1.018, y: -2, transition: { type: 'spring', stiffness: 300, damping: 20 }, }} >
                                <div className="flex gap-4 items-start p-5">
                                    ...
                                    <div className="flex-1 min-w-0 space-y-2">
                                        ...
                                        <span style={{ display: 'inline-block', fontSize: '0.68rem', fontWeight: 700, padding: '2px 10px', background: 'rgba(192,38,211,0.08)', color: 'var(--accent)', border: '1px solid rgba(192,38,211,0.25)', borderRadius: 'var(--radius-sm)', letterSpacing: '0.06em', }}>
                                            {cert?.institute}
                                        </span>
